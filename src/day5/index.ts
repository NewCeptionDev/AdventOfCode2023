...
const translateAgainstSingleMapWithRanges = (
  range: NumberRange,
  map: RangeMap
): { changed: NumberRange[]; unchanged: NumberRange[] } => {
  if (range.end < map.sourceStart || range.start > map.sourceStart + map.range - 1) {
    return {
      unchanged: [range],
      changed: [],
    }
  ...
  if (range.start >= map.sourceStart && range.end <= map.sourceStart + map.range - 1) {
      ...
      changed: [
        {
          ...
          end: map.destinationStart + map.range - 1,
        },
      ],
      unchanged: [
        {
...
const translateWithRanges = (ranges: NumberRange[], maps: RangeMap[]): NumberRange[] => {
  let updatedRanges = ranges
  const finalRanges = []

  // Apparently there is some overlap and the maps should be traversed from lowest sourceStart upwards
  const sortedMaps = maps.sort((a, b) => a.sourceStart - b.sourceStart)

  sortedMaps.forEach((map) => {
    const newRanges = []
    updatedRanges.forEach((range: NumberRange) => {
