...
type Cache = Map<string, {stones: Position[], index: number}>

const rollNorth = (stopper: Position[], stones: Position[], cache: Cache, currentColumn: number): Position[] => {
...
const rollSouth = (stopper: Position[], stones: Position[], columnHeight: number, cache: Cache, currentColumn: number): Position[] => {
...
const rollWest = (stopper: Position[], stones: Position[], cache: Cache, currentRow: number): Position[] => {
...
const rollEast = (stopper: Position[], stones: Position[], columWidth: number, cache: Cache, currentRow: number): Position[] => {
...
const fullRoll = (stopper: Position[], stones: Position[], direction: Direction, columnHeight: number, rowWidth: number, cache: Cache): Position[] => {
  let newStones = []

  const cacheString = "F;" + direction + ";" + stones.map(stone => stone.x + "." + stone.y).sort((a, b) => a.localeCompare(b)).join(",")
...
const fullCycle = (stopper: Position[], stones: Position[], columnHeight: number, rowWidth: number, cache: Cache, index: number): {
  stones: Position[],
  loopRange: number
} => {
  const cacheString = "C;" + stones.map(stone => stone.x + "." + stone.y).sort((a, b) => a.localeCompare(b)).join(",")

  if(cache.has(cacheString)) {
    console.log("found loop")
    return { stones: cache.get(cacheString).stones, loopRange: index - cache.get(cacheString).index }
  }

  const stonesAfterNorth = fullRoll(stopper, stones, Direction.N, columnHeight, rowWidth, cache)
  const stonesAfterWest = fullRoll(stopper, stonesAfterNorth, Direction.W, columnHeight, rowWidth, cache)
  const stonesAfterSouth = fullRoll(stopper, stonesAfterWest, Direction.S, columnHeight, rowWidth, cache)
  const stonesAfterEast = fullRoll(stopper, stonesAfterSouth, Direction.E, columnHeight, rowWidth, cache)
...
const calculateLoad = (stones: Position[], columnHeight: number): number => {
  return stones.map(stone => columnHeight - stone.y).reduce((previousValue, currentValue) => previousValue + currentValue)
}
...
const goB = (input: string) => {
  ...
  const stones = parseStonePositions(lines)
  ...
  for(let i = 0; i < 1000000000; i++) {
    ...
    if(result.loopRange !== -1) {
      const skipable = Math.floor((1000000000 - i) / result.loopRange)
      i += skipable * result.loopRange
    }


    if(i % 10000000 === 0) {
      console.log("Current", i / 10000000)
    }
  }
